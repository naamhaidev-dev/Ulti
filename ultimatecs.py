#!/usr/bin/env python3
"""
⚡ GITHUB CODESPACE BOT v5.1 – ALL COMMANDS FIXED
├─ All commands working: zip, grep, preview, getfile, findfile
├─ No duplicate messages
├─ Thread-safe for multiple users
└─ Dark UI with professional look
"""
...
def github_headers(token: Optional[str] = None) -> Dict[str, str]:
    h = {"Accept": "application/vnd.github+json", "X-GitHub-Api-Version": "2022-11-28", "User-Agent": "CodespaceBot/5.1"}
    if token:
...
def cmd_tokens(message):
    ...
    safe_send(message.chat.id, f"🔑 Token: `{masked}`", parse_mode="Markdown")
...
def cmd_files(message):
    ...
    safe_edit(msg, f"📂 *{owner}/{repo}* / `{final_path or '/'}`\nLoaded {len(tree)} files.", parse_mode="Markdown")
...
def cmd_grep(message):
    ...
    safe_edit(msg, output[:4000], parse_mode="Markdown")
...
def cmd_stats(message):
    ...
    conn.close()
    safe_send(message.chat.id, f"📊 *Stats*\nActions: `{actions}`\nCache: `{tree_cache.size()}`", parse_mode="Markdown")
...
def handle_callback(call: CallbackQuery):
    ...
    if data == "menu_codespaces":
        ...
        safe_edit(call.message, f"🐙 *Your Codespaces*\n{UI['separator']}", parse_mode="Markdown", reply_markup=markup)
        bot.answer_callback_query(call.id)
        return
...
{UI['separator']}
Click below to go back 👇""", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Main", callback_data="menu_main")]]))
        bot.answer_callback_query(call.id)
        return
...
🧹 /clear - Clear session
❓ /help - Full help"""
        safe_edit(call.message, text, parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Main", callback_data="menu_main")]]))
        bot.answer_callback_query(call.id)
        return
...
{UI['separator']}"""
    ...
    if data.startswith("cs_"):
        ...
        if not cs:
            ...
            bot.answer_callback_query(call.id)
            return
        safe_edit(call.message, format_codespace(cs), parse_mode="Markdown", reply_markup=create_codespace_menu(name, cs.get("state", "Unknown")))
        bot.answer_callback_query(call.id)
        return
...
if __name__ == "__main__":
    logger.info("🚀 Ultimate GitHub Bot v5.1 started polling...")
    logger.info("✅ All commands fixed!")
    try:
